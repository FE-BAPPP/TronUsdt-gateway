...
import { 
  ArrowLeft, User, DollarSign, Clock, Calendar, 
  CheckCircle, AlertCircle, MessageSquare, FileText,
  Briefcase, Star
} from 'lucide-react';
...
export function ProjectDetailPage() {
  ...
  const handleReleaseMilestone = async (milestoneId: string) => {
    ...
    try {
      await projectApi.releaseMilestone(milestoneId);
      alert('Milestone payment released successfully!');
      loadProjectDetails(); // Refresh
    } catch (error: any) {
      alert(error.response?.data?.message || 'Failed to release milestone');
    }
  };
  ...
  return (
    <div className="p-6 space-y-6">
                ...
                >
                  <div className="flex items-start justify-between">
                    ...
                    <div className="text-right ml-4">
                      ...
                      {/* Release Button (Employer only, for PENDING milestones) */}
                      {isEmployer && milestone.status === 'PENDING' && (
                        <button
                          onClick={() => handleReleaseMilestone(milestone.id)}
                          className="px-4 py-2 bg-green-500/20 text-green-400 rounded-lg hover:bg-green-500/30 transition-colors flex items-center gap-2 ml-auto"
                        >
      ...
      {/* Communication Section (Placeholder for future chat) */}
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
